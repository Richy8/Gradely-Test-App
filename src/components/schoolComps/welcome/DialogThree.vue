<template>
<div>
    <!-- BANNER -->
    <div class="banner">
        <div class="banner-overlay box-overlay brand_tonic_bg rounded-top-10">
            <img src="@/assets/images/DisplayBanner.png" alt="Display Banner" class="img img-luminous">
        </div>
        <img src="@/assets/images/DialogThree.png" alt="Remote Classroom" class="display-img">
    </div>

    <!-- WELCOME BLOCK -->
    <div class="welcome-block w-100 d-flex flex-column justify-content-start align-items-center">
        <div class="welcome-title font-weight-bold brand_navy">Online Remote Classroom</div>
        <div class="welcome-text color_grey_dark text-center">Lorem ipsum, or lipsum as it is sometimes known, is dummy text used in laying out print, graphic or web designs.</div>
    </div>
</div>
</template>

<script>
export default {
    name: "DialogThree"
}
</script>

<style>

</style>
